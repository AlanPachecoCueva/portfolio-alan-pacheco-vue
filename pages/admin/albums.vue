...
import { countImagesByAlbum } from '~/utils/firebase/images.js'
...
export default {
  ...
  data() {
    return { showForm: false, editingId: null, formName: '', formDescription: '' }
  },
  methods: {
    startEdit(album) {
      this.editingId = album.id; this.formName = album.name; this.formDescription = album.description || ''; this.showForm = true
    },
    cancelForm() { this.showForm = false; this.editingId = null; this.formName = ''; this.formDescription = '' },
    async handleSave() {
      if (!this.formName.trim()) return
      const data = { name: this.formName.trim(), description: this.formDescription.trim() }
      try {
        if (this.editingId) { await this.updateAlbum(this.editingId, data) } else { await this.createAlbum(data) }
        this.cancelForm()
    ...
    async handleDelete(album) {
      try {
        ...
        if (count > 0) {
          this.$swal.fire({ icon: 'warning', title: 'Álbum con imágenes', text: `No se puede eliminar "${album.name}" porque tiene ${count} imagen${count === 1 ? '' : 'es'} asociada${count === 1 ? '' : 's'}.` })
          return
        }
        const { isConfirmed } = await this.$swal.fire({
          icon: 'warning', title: '¿Eliminar álbum?', text: `Se eliminará "${album.name}" permanentemente.`,
...
.inline-form {
  background: #1e1e1e; padding: 20px; border-radius: 10px; margin-bottom: 24px;
  display: flex; flex-direction: column; gap: 12px; max-width: 480px;
}

.inline-form input {
  padding: 10px 14px; background: #2a2a2a; border: 1px solid #333;
  border-radius: 8px; color: #fff; font-size: 0.95em;
}

.inline-form input:focus { outline: none; border-color: #f39c12; }

.inline-form-actions { display: flex; gap: 10px; }

.btn-cancel { padding: 8px 16px; border-radius: 6px; background: #333; color: #fff; border: none; cursor: pointer; }
</style>
